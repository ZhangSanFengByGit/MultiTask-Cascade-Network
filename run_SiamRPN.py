...
def tracker_eval(net, x_crop, target_pos, target_sz, window, scale_z, p):
    ...
    return target_pos, target_sz, score[best_pscore_id], x_feat
...
def SiamRPN_track(state, im):
    ...
    s_z = np.sqrt(wc_z * hc_z)   #the true size of current z
    scale_z = p.exemplar_size / s_z
    d_search = (p.instance_size - p.exemplar_size) / 2   #p.instance sz and exemplar sz is only used for comparative
    pad = d_search / scale_z
    s_x = s_z + 2 * pad   #the true size of current x

    # extract scaled crops for search region x at previous target position
    x_crop = Variable(get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans).unsqueeze(0))

    target_pos, target_sz, score, x_feat = tracker_eval(net, x_crop.cuda(), target_pos, target_sz * scale_z, window, scale_z, p)
