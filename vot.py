...
class VOT(object):
    ...
    def __init__(self, region_format, vot_name):
        """ Constructor

        Args:
            region_format: Region format options
        """
        self.base_path = '/home/zhangzichun/MultiTask/vot2016/'+vot_name+'/'
        ...
        if TRAX:
            ...
            self._files = [x.strip('\n') for x in open(self.base_path+'images.txt', 'r').readlines()]
            self._frame = 0
            self._region = convert_region(parse_region(open(self.base_path+'region.txt', 'r').readline()), region_format)
            self._result = []
